...
func _on_body_entered(body: Node) -> void:
	...
	# Player should either be in group "player" or have a take_damage() function
...
func _on_area_entered(area: Area2D) -> void:
	# Optional: destroy projectile on shields/walls/hitboxes
	if area == owner_node:
		return

	if area.is_in_group("player_shield"):
		queue_free()
	elif area.is_in_group("solid_hitbox"):
		queue_free()
